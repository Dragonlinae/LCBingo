...
socket.on('update square', (questionIndex, playerIndex, timestamp) => {
    board.timestamp[questionIndex][playerIndex] = timestamp;
    updateSquareColor(document.getElementById('bingoBoardContainer').children[questionIndex], board.timestamp[questionIndex][0], board.timestamp[questionIndex][1], board.winCondition);
});
...
function updateSquareColor(square, player1Time, player2Time, winCon) {
    if (winCon == 2) {
...
function modifySquareGrid(gridContainer, size) {
...
const crownImage = document.getElementById("crown");
function animateWin(winner) {
    jsConfetti.addConfetti({
        confettiRadius: 10,
        confettiNumber: 500,
    });
    let targetPlayerImage = document.getElementById(`p${winner + 1}Avatar`);
...
const drawImage = document.getElementById("draw");
