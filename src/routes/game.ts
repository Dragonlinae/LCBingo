...
io.of('/game').on('connection', (socket) => {
  ...
  socket.on('check solves', () => {
    const room: string = Array.from(socket.rooms).filter(
      (r) => r !== socket.id,
    )[0];
    if (rooms.has(room)) {
      function updateClients(
        ...
        io.of('/game')
          .to(room)
          .emit('update square', questionIndex, playerIndex, timestamp);
      }
...
async function newRoom(board: Board): Promise<string> {
  if (rooms.size >= maxRoomCount) {
    ...
    if (!isRoomDeleted) {
      console.log(
        'Cannot create new room, maximum limit reached and no room ' +
  ...
  try {
    ...
    throw new Error(
      'Failed to fetch player images. Please check player names.',
    );
  }
  console.log(board);
